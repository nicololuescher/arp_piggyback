...
def main(interface, descriptor, verbose, is_reply):
    ...
    if len(arp_cache) == 0:
        ...
        random_ip = random.choice(arp_cache)
    ...
    message = str(int(time.time())) + descriptor
    hd = bytes(message, "utf-8")
    ...
    for i in range(math.ceil(len(encrypt) / (60 - len(packet_header) - 2))):
        packet = (
            ...
            + encrypt[
                i
                * ((60 - len(packet_header) - 2)) : (i + 1)
                * ((60 - len(packet_header) - 2))
            ]
        )
        # send packet
        if verbose:
            print(
                ...
                i + 1,
                "of",
                math.ceil(len(encrypt) / (60 - len(packet_header) - 2)),
            )
